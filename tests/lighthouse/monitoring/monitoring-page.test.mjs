...
const TEST_TIMEOUT_MS = Number(process.env.LIGHTHOUSE_MONITORING_TIMEOUT_MS || 600_000);

const THRESHOLDS = {
  performance: readThreshold('LIGHTHOUSE_MONITORING_PERFORMANCE_MIN', 0.7),
  accessibility: readThreshold('LIGHTHOUSE_MONITORING_ACCESSIBILITY_MIN', 0.9),
  'best-practices': readThreshold('LIGHTHOUSE_MONITORING_BEST_PRACTICES_MIN', 0.9),
  seo: readThreshold('LIGHTHOUSE_MONITORING_SEO_MIN', 0.8),
};
...
test('Monitoring page meets Lighthouse budgets', { timeout: TEST_TIMEOUT_MS }, async () => {
  ...
  try {
    ...
    await saveReports(result.lhr, 'monitoring-page');
...
function resolveMockApiConfig() {
  const rawUrl =
    process.env.LIGHTHOUSE_MONITORING_MOCK_API_URL ||
  ...
  if (url.protocol !== 'http:' || !isLocalHost) {
    throw new Error('LIGHTHOUSE_MONITORING_MOCK_API_URL must be an http localhost URL');
  }

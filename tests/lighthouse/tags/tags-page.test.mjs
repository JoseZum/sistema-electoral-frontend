...
const TEST_TIMEOUT_MS = Number(process.env.LIGHTHOUSE_TAGS_TIMEOUT_MS || 600_000);

const THRESHOLDS = {
  performance: readThreshold('LIGHTHOUSE_TAGS_PERFORMANCE_MIN', 0.7),
  accessibility: readThreshold('LIGHTHOUSE_TAGS_ACCESSIBILITY_MIN', 0.9),
  'best-practices': readThreshold('LIGHTHOUSE_TAGS_BEST_PRACTICES_MIN', 0.9),
  seo: readThreshold('LIGHTHOUSE_TAGS_SEO_MIN', 0.8),
};
...
async function saveReports(lhr) {
  await mkdir(REPORT_DIR, { recursive: true });
  await writeFile(path.join(REPORT_DIR, 'tags-page.lhr.json'), JSON.stringify(lhr, null, 2));
  await writeFile(path.join(REPORT_DIR, 'tags-page.report.html'), generateReport(lhr, 'html'));
}
...
function assertFinalPage(lhr, expectedUrl) {
  assert.equal(
    ...
    'Lighthouse did not audit the expected /tags page'
  );
}
